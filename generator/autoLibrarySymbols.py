# autoLibrarySymbols.py
#
# Adapted from CDFER/JLCPCB-Kicad-Library's autoLibrarySymbols.py (MIT
# licensed, Copyright (c) 2023 Chris). See PORTING.md for what changed:
# geometry helpers and the transistor/inductor/fuse body literals are kept
# verbatim; generate_kicad_symbol()'s mode dispatch and property schema are
# rewritten to key off ES catc codes and the ES field schema instead of
# JLCPCB's category taxonomy and LCSC-based fields.
...
def generate_kicad_symbol(
    ...
    if mode == "Resistors":
        ...
        value_autoplace = True
        name = f"{footprint},{value}"
        print(f"Error: Unknown autoLibrarySymbol mode for {es_url} ({mode})")
    ...
    # CDFER's original suffixing capped at ",(6)" and silently fell through
    # to a colliding name past that -- fine at JLCPCB's basic/preferred-tier
    # scale, but ES's full catalogue routinely has 7+ vendors selling the
    # same {package},{value} combo (e.g. 0402 10kOhm resistors), so an
    # unbounded counter replaces the fixed ",(2)".."(6)" ladder. A silent
    # name collision means the later symbol is unreachable in KiCad (its
    # library file parser keeps only the last symbol with a given name),
    # so this has to never fall through.
    if name in names_lookup:
        suffix_n = 2
        while f"{name},({suffix_n})" in names_lookup:
            suffix_n += 1
        name = f"{name},({suffix_n})"

    names_lookup.append(name)

    # No PCM_JLCPCB:-style namespace prefix here: Route A footprints resolve
    # against KiCad's own stock libraries (bare LIB_ID), Route B against
    # ES.pretty. See plan.md §3.2-3.3; footprints_lookup carries the actual
    # resolved LIB_ID per footprint token rather than a fixed prefix.
    footprint = footprints_lookup.get(f"{ref_designator}_{footprint}", f"ES:{ref_designator}_{footprint}")
    if mode == "Transistors":
    ...
    symbol += generate_property("ES_PN", es_pn, "0 0 0")
    symbol += generate_property("ES_URL", es_url, "0 0 0")
    symbol += generate_property("Stock", stock, "0 0 0")
    symbol += generate_property("Stock_Ratchaphruek", stock_ratchaphruek, "0 0 0")
    symbol += generate_property("Stock_BanMo", stock_banmo, "0 0 0")
    symbol += generate_property("Stock_Date", stock_date, "0 0 0")
    symbol += generate_property("Price", price, "0 0 0")
    symbol += generate_property("Min Order Qty", min_order_qty, "0 0 0")
